...
方案发布时生成任务内的两轮选款页：先看 10 张代表展示图，再看所选风格的全部现有档位与配色。技能包只包含在线图片索引，31 张精确组合预览和 10 张代表图均从 imgtg 加载；缺少立体 1 档中线、玻璃 1 档偏冷、玻璃 2 档偏冷时会明确说明，组合仍可选择且不会自动补生图。
...
两轮选款（全部风格 → 所选风格的全部档位与配色组合）及加载在线图片的本地选择页已在当前源码实现并通过静态、源码、编译产物和独立打包安装测试；尚未发布。浏览器点击、剪贴板、窄视口和手机呈现仍未完成真实交互验收。
