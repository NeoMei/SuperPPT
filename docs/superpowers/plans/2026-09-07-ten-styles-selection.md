...
- [ ] Extract each new recipe directly from its accepted prompt: replace the palette paragraph, the content-relationship paragraph, and full visible-copy block with the respective slots. Strip the trailing fixture-specific usage note. Change only the fixture phrase “中文 PPT 广告展示页” to “中文 PPT 页面” as needed for general use. Keep all remaining art/tier instructions, including the accepted collage wording, intact. Record these transformations and source hashes in provenance.
