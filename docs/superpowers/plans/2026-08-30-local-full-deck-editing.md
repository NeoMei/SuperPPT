...
The guided route has an authenticated `style-selection` stage between `slide-specs` and sample authorization. Persist the user's single selection with the exact current project revision and representative stable slide ID by running `style-selection --project <root> --input <selection.json>`; the resulting v2 evidence must bind the Style Lock SHA produced from that same selection. Legacy v1 evidence is read/migration-only and cannot publish a sample-generation plan. Sample generation authorization must fail closed until the authenticated v2 binding exists. Do not apply a default style or infer a representative page.
