...
The guided route has an authenticated `style-selection` stage between `slide-specs` and sample authorization. Persist the user's single selection with the exact current project revision and representative stable slide ID by running `style-selection --project <root> --input <selection.json>`; the resulting canonical v2 evidence must bind the Style Lock SHA produced from that same selection, and `manifest.style` must bind the actual selection bytes. A matching current v1 selection is migrated atomically only by this public route; v1 itself cannot publish a sample-generation plan. Exact retries must recover interruptions after lock publication, selection publication, or immediately before manifest stage update, while any mismatched v1/v2 selection, lock, or revision fails closed without modifying the evidence. A revision race may retire stale evidence only after proving the lock belongs to the current project and a strictly older revision, its direct child `parentSnapshotDescriptorSha256` authenticates that revision's immutable snapshot-v2 descriptor, and the exact snapshot copies bind selection, lock, and recipe transitively. Retirement is a persisted replayable transaction that converges after each of the three removal boundaries; do not use independent unjournaled unlinks. Project rollback restores those exact three files, `manifest.style`, and the `style-selection` stage from the same descriptor-bound snapshot even when no style-sample gate exists. Legacy v1 snapshots are readable only when no style chain is claimed; missing, linked, extra, or hash-conflicting snapshot evidence fails closed without live mutation. Self-consistency or revision membership alone never authorizes deletion. Sample generation authorization must fail closed until the authenticated v2 binding exists. Do not apply a default style or infer a representative page.

Every complete-deck editing cycle begins from an authenticated `deck-review` action. Run `complete-deck-review --project <root> --action edit-page --revision-id <current-revision-id> --sha256 <current-sha256> --slide-id <stable-slide-id>` before preparing the edit. This persists one pending binding to that exact current revision/SHA/stable slide. Manual and Agent preparation must atomically consume it and must reject missing, wrong-slide, stale, or replayed bindings before creating any candidate/session, leaving zero residue; direct candidate creation cannot bypass this boundary. Manual adoption, Agent confirmation, Agent rejection, and pointer-only rollback all clear pending state, return the project to `deck-review`, clear stale formal-delivery/client/export bindings, and bind any following action to the new exact current revision and SHA-256.
