...
After the human has actually completed those actions, write the strict observation to a private `0600` input with `application`, `smokeCopyDescriptorPath`, non-empty `selectedObject`, `temporaryEditObserved:true`, `undoObserved:true`, `saveDecision:"discarded"`, `reopenObserved:true`, the physically re-read `reopenedCopySha256`, `observedResult`, and `confirmedAt`. Then run `npm run cli -- acceptance-record --project <root> --input <file>`. Only a successful authenticated record is the formal delivery boundary. Never fabricate an observation or infer it from the unchanged hash alone.

Never open or save canonical `output/revisions/<n>/deck.pptx` for smoke testing, and never tell the client to save the smoke edit. If client control is unavailable, report acceptance pending without invoking `acceptance-record`. On any failure, keep the previous deliverable unchanged and report the exact resumable stage.
